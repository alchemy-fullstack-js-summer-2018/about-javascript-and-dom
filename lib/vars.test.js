describe.skip('Variables, Arguments, Values, and References suite', () => {
  ...
  it('Passing a var with obj val as an arg to a func copies the obj ref to the corresponding param and reassigning the arg in the func doesnt alter the original var.', () => {
    ...
    const result = copyObject(anObj, anotherObj);
    expect(result).not.toBe(anObj);
  });

  it('Passing a varithat holds an objas an arg to a func and then assigning a propto the obj inside the func will alter the original obj to have the new prop val.', () => {
    const anObj = { smell: 'rage' };
    addProp(anObj, 'spicy');
