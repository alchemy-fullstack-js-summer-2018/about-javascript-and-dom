describe('Testing Variables, Arguments, Values, and References', () => {
  ...
  it.skip('Passing a variable that holds a number as an argument to a function copies the value to the corresponding parameter arguments and reassigning the argument in the function will not alter the original variable value', () => {
  ...
  it('Passing a variable that holds an object as an argument to a function copies the object reference to the corresponding parameter arguments and reassigning the argument in the function will not alter the original variable', () => {
    ...
    function verify(obj) {
      expect(obj).toEqual({ name: 'Wilford' });
      obj = { type: 'Pacific' };
    }
    verify(walrus);
    expect(walrus).toEqual({ name: 'Wilford' });
  }),
