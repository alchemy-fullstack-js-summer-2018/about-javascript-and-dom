...
const { TextComponent } = require('./javascript');
const { MyElement } = require('./javascript');
...
describe('TextComponent tests', () => {
  ...
  let message = new TextComponent('Hello there!'); 
  it('takes an optional initial value in its constructor(text)', () => {  
    expect(message.text).toBe('Hello there!');
  });

  it('returns TextNode that has data property set to initial text value, or nothing', () => {
    let textNode = message.render();
    expect(textNode.data).toBe('Hello there!');
  });

  it('updates text via data property', () => {
    let textNode = message.updateText('Hi');
    expect(textNode.text).toBe();
  });

  it('calls render a second time and returns same textNode', () => {
    let textNode = message.render();
...
describe('ElementClasses tests', () => {
  ...
  let elemento = new MyElement(this.element);

  it('takes an element in its constructor(element)', () => {
    expect(elemento.element instanceof HTMLElement).toBeTruthy();
  });

  it('throws error if no argument or not instance of HTMLElement', () => {  
    function noArg() {
  ...
  it('has an add className method that adds className to element', () => {
    elemento.add('Yipee');
    elemento.add('Crazy');
    expect(elemento.element.classList[0]).toBe('Yipee');
  });

  it('has a removeClassName method that removes className', () => {
    elemento.remove('Yipee');
    expect(elemento.element.classList[0]).toBe('Crazy');
  });
});