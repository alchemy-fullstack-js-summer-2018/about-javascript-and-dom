...
describe('Variables, Arguments, Values, References', () => {
  ...
  it('does not alter original variable when argument holds a number and gets reassigned', () => {
    let pct = 2650;
    function fireClosure(x) {
      return x + 375;
    }
    fireClosure(pct);
    
    expect(pct).toBe(2650);
  });

  it.skip('does not alter original variable when argument holds an object that gets reassigned', () => {
    let hikerTrash = {
      trailName: 'Dirty Avocado',
      realName: 'Maria'
    };
    
    function realWorld(x) {
      x = {
        realName: 'Maria Cotezi'
      };
      realWorld(hikerTrash);
    }

    expect(x.realName).toBe('Maria');
  });

  it('alters the original object when assigning a property to the object inside the original function parameter', () => {
    let hikerTrash = {
      trailName: 'Twerk In The Dirt',
      realName: 'Dave'
    };
    
    function realWorld(hikerTrash) {
      hikerTrash.realName = 'Tommy Corey';
    }
    realWorld(hikerTrash);

    expect(hikerTrash.realName).toBe('Tommy Corey');
